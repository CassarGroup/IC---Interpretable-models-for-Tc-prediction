...
def create_instance(trial):
    ...
    if distribution_name == "gamma":
        ...
        link_name = trial.suggest_categorical(
            "link_inverse_gaussian", ["log", "inverse-squared"]
        )
    
    lam = trial.suggest_loguniform("lam", 1e-3, 1e2)
...
def optimization(X_train, y_train):
    ...
    clusters_study = create_study(
        direction="minimize",
        study_name=f"optimization_clusters_gam_teste56",
        storage=f"sqlite:///optimization_clusters_gam_teste56.db",
        load_if_exists=True,
